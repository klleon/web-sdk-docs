...
<Head>
  <title>{`Klleon Chat SDK 이벤트 처리 - onChatEvent, onStatusEvent, onErrorEvent 사용법`}</title>
  <meta
    name="description"
    content="Klleon Chat SDK에서 제공하는 onChatEvent, onStatusEvent, onErrorEvent를 활용해 아바타 상태 변화, 채팅 메시지, 에러를 처리하는 방법을 안내합니다. ErrorCode별 발생 조건과 에러 이벤트 흐름까지 상세하게 정리되어 있습니다."
  />
  <meta
    name="keywords"
    content="Klleon Chat SDK, 이벤트 처리, onChatEvent, onStatusEvent, status 이벤트, VIDEO_CAN_PLAY, 채팅 이벤트, chat_type, STATUS 흐름도, 디지털 휴먼 SDK, 클레온 SDK, 실시간 채팅 이벤트, 자바스크립트 이벤트 리스너, onErrorEvent, 에러 이벤트"
  />
</Head>
...
flowchart TD
    ...
    D -->|성공| H[CONNECTED_FINISH]
    D -->|실패| J[STREAMING_FAILED]
    H --> F[VIDEO_LOAD]
    F --> G[VIDEO_CAN_PLAY]
...
- **callback** (`(error: ErrorData) => void`, 필수): `ErrorData` 객체(`{ code: ErrorCode; message: string }`)를 인자로 받는 콜백 함수입니다.
...
<Table columns={usage.ko.errorCodeColumns} data={usage.ko.errorCodeData} />
...
flowchart TD
    subgraph INIT["초기화 단계"]
        A["KlleonChat.init()"] --> B{WebSocket 연결}
        B -->|실패| C["SOCKET_FAILED"]
        B -->|성공| D{스트리밍 연결}
        D -->|"실패 / 타임아웃(10초)"| E["STREAMING_FAILED"]
        D -->|성공| F["VIDEO_CAN_PLAY ✓"]
    end

    subgraph RUNTIME["런타임 단계"]
        F --> G{연결 모니터링}
        G -->|"avatar-container 미발견"| H["VIDEO_ELEMENT_NOT_FOUND"]
        G -->|"WebSocket 에러"| I["SOCKET_DISCONNECTED_UNEXPECTEDLY"]
        G -->|"스트리밍 재연결 실패"| J["STREAMING_RECONNECT_FAILED"]
        G -->|"스트리밍 연결 끊김"| K["STREAMING_DISCONNECTED_UNEXPECTEDLY"]
    end
...
:::info 콜백 재등록
`onChatEvent`, `onStatusEvent` 또는 `onErrorEvent`를 동일한 이벤트 타입에 대해 여러 번 호출하면, 마지막으로 등록된 콜백 함수만 활성화됩니다. 즉, 새 콜백을 등록하면 이전 콜백은 자동으로 해제됩니다. 명시적인 `off` 메서드는 제공되지 않습니다.
:::
