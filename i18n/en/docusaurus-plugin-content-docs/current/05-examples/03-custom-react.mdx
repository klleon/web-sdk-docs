...
Add the following script tag inside the `<head>` section of your `public/index.html`. Replace `{VERSION}` with your actual SDK version (e.g., 1.3.0).
  ...
  .klleon-chat-container {
    ...
    .chat-container {
      ...
      .chat-item {
        ...
        flex-direction: column;
        gap: 12px;
        background: var(--klleon-color-text-muted);
        color: #fff;
        padding: 12px;
        border-radius: 12px;
      }
...
```tsx title="App.tsx"
// Types are provided by KlleonSDK.d.ts. (See the TypeScript Guide)
import type { ChatData, ResponseChatType, Status } from "./KlleonSDK";
...
const App = () => {
  ...
  const sdkHandler = {
    startChat: async () => {
      ...
      KlleonChat.onErrorEvent((error: { code: string; message: string }) => {
        console.error("SDK Error Event:", error.code, error.message);
      });
